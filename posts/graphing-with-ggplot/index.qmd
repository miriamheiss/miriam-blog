...
Let's start graphing! In `aes()`, choose your x, your y, and your color. I am going to be using height for x, weight for y, and sex for color.
...
```
That looks much nicer. But what if we want to separate the sex so they don't touch? 
...
```{r facet_wrap_sex, warning = FALSE}
ggplot(data = nhanes,
       mapping = aes(x = height,
                     y = weight,
                     color = sex)) +
...
ggplot(data = nhanes,
  ...
  labs(title = "Heights in the U.S.",
       ...
       y = "Weight (kg)")
```
Awesome! But what about the "sex" above the legend? 

In the `aes()` we set x equal to height, y equal to weight, and color equal to sex. We changed x and y, and we can change "sex" the same way! We can set color equal to "sex", and that will fix the lowercase "sex" on the legend.
